...
<!--  $Id: free_node.c.xsl 2501 2009-08-03 10:10:09Z jju $  -->
...
<xsl:template match="/">
  ...
  <xsl:call-template name="travfun-file">
    ...
    <xsl:with-param name="xslt">
      <xsl:value-of select="'$Id: free_node.c.xsl 2501 2009-08-03 10:10:09Z jju $'"/>
    </xsl:with-param>
  </xsl:call-template>
...
<xsl:template match="node">
  ...
  <xsl:value-of select="' = NULL;'" />
  <!-- Free err_code -->
  <xsl:value-of select="'NODE_ERRCODE(arg_node) = FREEattribString(NODE_ERRCODE(arg_node), arg_node);'"/>
